...
function BillingDrawerWrapper({ handleClose, open, content }) {
    console.log(open);
    ...
    return (
        <Drawer variant='temporary' anchor='right' containerStyle={{ height: 'calc(100% + 64px)' }} PaperProps={{
            ...
            <div style={{ height: 1000 }}>
                {
                    content === "NewBill" && <NewUser close={handleClose} />
                }
                {
                    content === "EditBill" && <EditBilling close={handleClose} />
                }
            </div>
        </Drawer>
    )
...
export default BillingDrawerWrapper